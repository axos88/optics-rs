use crate::lens::{ComposeWithLens, Lens, mapped_lens};
use crate::prism::ComposeWithPrism;
use crate::{NoFocus, Optic, Prism, mapped_prism};
...
fn read_value_using_prism() {
    let config = Config::default();
    let main_prism = mapped_prism(|c: &Config| Some(c.main.clone()), |c, v| c.main = v);
    assert_eq!(main_prism.preview(&config), Some(config.main));
}
...
fn can_combine_prisms() {
    ...
    let main_prism = mapped_prism(|c: &Config| Some(c.main.clone()), |c, v| c.main = v);
    let port_prism = mapped_prism(|c: &DatabaseConfig| c.port, |c, v| c.port = Some(v));
...
fn can_combine_lens_with_prisms() {
    ...
    let port_lens = mapped_lens(|c: &DatabaseConfig| c.port, |c, v| c.port = v);
    let main_prism = mapped_prism(|c: &Config| Some(c.main.clone()), |c, v| c.main = v);
    let port_prism = mapped_prism(|c: &DatabaseConfig| Some(c.port), |c, v| c.port = v);
    ...
    assert_eq!(composed1.preview(&config), Some(config.main.port));
    assert_eq!(composed2.preview(&config), Some(config.main.port));
}
