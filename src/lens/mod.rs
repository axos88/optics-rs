...
pub trait Lens<S, A>:
    HasPartialGetter<S, A, GetterError = Infallible> + HasGetter<S, A> + HasSetter<S, A>
{
}

impl<S, A, L: HasPartialGetter<S, A, GetterError = Infallible> + HasGetter<S, A> + HasSetter<S, A>>
    Lens<S, A> for L
{
}
